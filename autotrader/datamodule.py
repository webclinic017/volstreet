...
class DataClient:
    ...
    def get_data(self, symbol, from_date='2011-01-01', return_columns=None):
        ...
        symbol_dict = {"NIFTY": "NSEI.INDX", "BANKNIFTY": "NSEBANK.INDX", "FINNIFTY": "CNXFIN.INDX"}
        symbol = symbol.upper()
        name = symbol.split('.')[0]
        if '.' not in symbol:
        ...
        resp = self.client.get_prices_eod(symbol, period='d', order='a', from_=from_date)
...
def analyser(df, frequency=None, date_filter=None, _print=False):
    ...
    if frequency is None or frequency == 'D' or frequency == 'B':
        ...
        if frequency.startswith('W'):
            ...
            if len(frequency) == 1:
                ...
                weekday_module_dict = {'MON': MO, 'TUE': TU, 'WED': WE, 'THU': TH, 'FRI': FR}
                frequency = frequency.lstrip('M-')
                df = df.resample(f'W-{frequency.upper()}').ffill()
                df = df.resample('M').ffill()
                df.index = df.index.date + relativedelta(weekday=weekday_module_dict[frequency.upper()](-1))
    ...
    if _print:
        print('Vol for period: {:0.2f}%, IV: {:0.2f}%'.format(df.abs_change.mean(), df.abs_change.mean() * multiplier))
    else:
        pass
...
def get_multiple_recent_vol(list_of_symbols, frequency, periods=None, ignore_last=1, client=None):
    ...
    for symbol in list_of_symbols:
        ...
        recent_vol = get_recent_vol(symbol_monthly_data, periods=periods, ignore_last=ignore_last)
...
def ratio_analysis(x_df, y_df, periods_to_avg, return_summary=False):
    ...
    ratio_summary = pd.DataFrame({labels[0]: x_close, f'{labels[0]} Change': x_array,
                                  labels[1]: y_close, f'{labels[1]} Change': y_array,
                                 'Ratio': ratio_array})
...
def generate_streak(df, query):
    ...
    df['start_of_streak'] = (df['result'].ne(df['result'].shift())) & (df['result'] == True)
...
def gambler(instrument, freq, query):
    ...
    def _calculate_streak_summary(dataframe, frequency):
        df = dataframe.reset_index().groupby('streak_id').agg({'date': ['min', 'max'],
                                                                      'streak_count': 'max'}).reset_index()
        df.columns = ['streak_id', 'start_date', 'end_date', 'streak_count']

        # Check if there is an ongoing streak
        current_streak = df.iloc[-1].streak_count if pd.Timestamp.today().date() <= df.iloc[-1].end_date.date() else False
    ...
    def print_streak_summary(streak_summary):
        print(f"{streak_summary['freq']} - longest streak: {streak_summary['longest_streak']} from {streak_summary['longest_streak_start']:%d %b %Y} to {streak_summary['longest_streak_end']:%d %b %Y}")
    ...
    return pd.concat(streaks_df).sort_values('longest_streak', ascending=False).reset_index(drop=True)
